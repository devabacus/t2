...
class TaskTagMapEndpoint extends Endpoint {
  ...
  Future<void> _notifyChange(Session session, TaskTagMapSyncEvent event, int userId) async {
  ...
  Future<void> _validateTaskAndTag(Session session, UuidValue taskId, UuidValue tagId, int userId) async {
    // Проверяем, что Task существует и принадлежит пользователю
    final task = await Task.db.findFirstRow(
      session,
      where: (t) => t.id.equals(taskId) & t.userId.equals(userId) & t.isDeleted.equals(false),
    );
    if (task == null) {
    ...
    final tag = await Tag.db.findFirstRow(
      session,
      where: (t) => t.id.equals(tagId) & t.userId.equals(userId) & t.isDeleted.equals(false),
    );
    if (tag == null) {
  ...
  Future<TaskTagMap> createTaskTagMap(Session session, UuidValue taskId, UuidValue tagId) async {
    ...
    return await session.db.transaction((transaction) async {
      ...
      final existingRelation = await TaskTagMap.db.findFirstRow(
        session,
        where: (r) => r.taskId.equals(taskId) & r.tagId.equals(tagId) & r.userId.equals(userId),
      ...
      if (existingRelation != null) {
        ...
        session.log('ℹ️ Воскрешаем удаленную связь Task($taskId) ↔ Tag($tagId)');
      ...
      session.log('✅ Создана связь Task($taskId) ↔ Tag($tagId) для пользователя $userId');
      return result;
    });
  }
...
Future<bool> deleteTaskTagMapByTaskAndTag(Session session, UuidValue taskId, UuidValue tagId) async {
    ...
    return await session.db.transaction((transaction) async {
        ...
        final relation = await TaskTagMap.db.findFirstRow(
            session,
            where: (r) => r.taskId.equals(taskId) & r.tagId.equals(tagId) & r.userId.equals(userId) & r.isDeleted.equals(false),
            transaction: transaction,
        );
