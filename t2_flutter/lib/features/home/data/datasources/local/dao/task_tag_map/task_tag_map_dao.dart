...
class TaskTagMapDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<bool> updateTaskTagMap(TaskTagMapTableCompanion companion, {required int userId, required String customerId}) async {
  ...
  Future<bool> softDeleteTaskTagMapById(String id, {required int userId, required String customerId}) async {
    ...
    Future<int> softDeleteRelationsByTaskId(String taskId, {required int userId, required String customerId}) async {
    final companion = TaskTagMapTableCompanion(
      isDeleted: Value(true),
      lastModified: Value(DateTime.now().toUtc()),
    );
    ...
    final updatedRows = await (update(taskTagMapTable)
          ..where((t) => t.taskId.equals(taskId) & t.userId.equals(userId) & t.customerId.equals(customerId)))
        .write(companion);
  ...
  Future<int> physicallyDeleteTaskTagMap(String id, {required int userId, required String customerId}) async {
  ...
  Future<TaskTagMapTableData?> getRelationById(String id, {required int userId, required String customerId}) {
...
Future<TaskTagMapTableData?> getRelationByTaskAndTag(String taskId, String tagId, {required int userId, required String customerId}) {
  return (select(taskTagMapTable)
  ...
  Stream<List<TaskTagMapTableData>> watchAllRelations({required int userId, required String customerId}) {
    return (select(taskTagMapTable)
          ..where((t) => t.userId.equals(userId) & t.customerId.equals(customerId) & t.isDeleted.equals(false)))
        .watch();
  }
}
