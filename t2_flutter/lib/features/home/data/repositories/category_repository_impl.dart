...
class CategoryRepositoryImpl extends BaseSyncRepository
    ...
    for (final localChange in localChangesToPush as List<CategoryTableData>) {
      if (localChange.syncStatus == SyncStatus.deleted) {
        try {
          ...
          await _localDataSource.physicallyDeleteCategory(localChange.id,
              userId: userId);
          print(
  ...
  Future<String> createCategory(CategoryEntity category) async {
    final categoryWithUser = category.copyWith(userId: userId);
  ...
  Future<bool> updateCategory(CategoryEntity category) async {
    final categoryWithUser =
        category.copyWith(userId: userId, lastModified: DateTime.now().toUtc());
    final result =
        await _localDataSource.updateCategory(categoryWithUser.toModel());
    syncWithServer().catchError(
  ...
  Future<bool> deleteCategory(String id) async {
    final result = await _localDataSource.deleteCategory(id, userId: userId);
    syncWithServer().catchError(
  ...
  Future<List<CategoryEntity>> getCategoriesByIds(List<String> ids) async {;
    final models = await _localDataSource.getCategoriesByIds(ids, userId: userId);
  ...
  Future<void> _syncCreateToServer(CategoryEntity category) async {
    try {
      ...
      await _localDataSource.insertOrUpdateFromServer(
          syncedCategory, SyncStatus.synced);
    } catch (e) {
      rethrow;
    }
  }
  ...
  Future<void> _syncUpdateToServer(CategoryEntity category) async {
    try {
      ...
      await _localDataSource.insertOrUpdateFromServer(
          serverCategory, SyncStatus.synced);
    } catch (e) {
      rethrow;
    }
  }
